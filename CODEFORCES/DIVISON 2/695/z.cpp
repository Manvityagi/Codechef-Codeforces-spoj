...
void update(V<int> &arr, V<int> &h, int index){
    int l = max(index-5,0);
    int r = min(index+5,(int)arr.size()-1);
    int i = l;
    while(i <= r){
        if(i > 0 && i+1 < arr.size()){
            h[i] = 0;
            h[i] |= (arr[i] > arr[i-1] && arr[i] > arr[i+1]);
            h[i] |= (arr[i] < arr[i-1] && arr[i] < arr[i+1]);
        }
        i++;
    }
...
int result(V<int> &h, int index){
    int l = max(index-5,0);
    int r = min(index+5,(int)h.size()-1);
    int ans = 0, i = l;
    while(i <= r){
        ans += h[i++];
    }
